---
title: "Compte Rendu TP Probabilités"
...
Remarque générale : Dans tous les diagrammes, on remarque que la derniÃ¨re série de valeurs apparait moins que les autres, tout simplement car cette plage de valeurs est moins large que les autres (Largeur normale : 2.0e08, largeur derniÃ¨re plage : 5.25e07). Ces valeurs ont donc logiquement moins de chances d'apparaÃ®tre.
...
On remarque que la répartition des valeurs est plutÃ´t équilibrée sauf pour la deuxiÃ¨me plage de valeurs qui n'apparaissent que 60 fois environ. Cela ne nous permet pas de conclure mais on peut supposer qu'il y a un possible désequilibre dans la répartition des valeurs.
...
La répartition des valeurs est équlibrée et ne traduit pas de problÃ¨me particulier.
...
La répartition des valeurs est parfaitement équilibrée et semble mÃªme "trop" réguliÃ¨re.
...
Pour les trois premiers algorithmes, il n'y a pas de remarque particuliÃ¨re si ce n'est que les valeurs consécutives semblent indépendantes contrairement Ã  la suite de Sobol oÃ¹ l'on observe une corrélation entre les valeurs obtenues. Il semble que celle-ci ne soit pas adaptée pour générer des valeurs imprévisibles.
...
# Simulations de lois de probabilités quelconques
## Question 6
...
FileMM1 <- function(lambda, mu, D)
{
  ...
  if (length(arrivee) > 0)
  {
    ...
    {
      if (arrivee[i] > depart [i-1]) # Quand le client i arrive, le client i-1 est déjà parti
        time = arrivee[i] + rexp(1, mu)
      else # Quand le client i arrive, il attend le départ de i-1
